<div class="card mb-4">
    <div class="card-body">
        ...
        <?php if($defaut->getPhoto()): ?>
            <img src="/uploads/defauts/<?= $defaut->getPhoto() ?>" alt="<?= htmlentities($defaut->getLieu()) ?>" class="w-50 card-img-top mt-3 mb-4">
        <?php endif ?>
        ...
        <a href="<?= $router->url('edit_defaut', ['id' => $defaut->getID()]) ?>" class="btn btn-warning">Modifier</a>
        <form action="<?= $router->url('edit_defaut_delete', ['id' => $defaut->getID()]) ?>" method="POST" onsubmit="return confirm('Voulez vous vraiment effectuer cette action ?')" class="d-inline">
